#!/usr/bin/env npx tsx
/**
 * Fetch real ETH historical data from APIs and save to files
 * This collects REAL price data (not synthetic) for ETH paper trading
 * 
 * Usage:
 *   pnpm tsx scripts/fetch-eth-real-historical-data.ts [timeframe] [startDate] [endDate]
 * 
 * Examples:
 *   pnpm tsx scripts/fetch-eth-real-historical-data.ts 8h 2020-01-01 2025-12-31
 *   pnpm tsx scripts/fetch-eth-real-historical-data.ts 4h 2020-01-01 2025-12-31
 */
...
async function main() {
  ...
  console.log(`🔄 Fetching REAL ETH historical data from APIs...`);
  ...
  try {
    ...
    // skipAPIFetch=false to force API fetch, allowSyntheticData=false to ensure REAL data only
    console.log('📡 Fetching from APIs (this may take a while)...');
    ...
    console.log(`✅ Fetched ${candles.length} REAL ${timeframe} candles from APIs`);
    ...
    console.log('   - ETH paper trading can now use this real historical data');
  } catch (error) {
