/**
 * Enhanced Adaptive Backtest Engine
 * Features:
...
export interface EnhancedAdaptiveBacktestOptions {
  startDate: string;
  endDate: string;
...
export interface EnhancedAdaptiveBacktestResult {
  run: StrategyRun;
  trades: Trade[];
...
/**
 * Run an enhanced adaptive backtest with persistence and dynamic sizing
 */
export async function runEnhancedAdaptiveBacktest(
  ...
  // Clear caches at start of backtest
  clearIndicatorCache();
  clearRegimeHistory();
  ...
  for (let i = 50; i < candles.length; i++) {
    ...
    const adaptiveSignal = generateEnhancedAdaptiveSignal(candles, config, i);
    ...
    const basePositionMultiplier = (adaptiveSignal.momentumConfirmed && 
                                 adaptiveSignal.activeStrategy === config.bullishStrategy)
      ? (config.bullishPositionMultiplier || 1.0)
      : 1.0;
    
    const finalPositionMultiplier = basePositionMultiplier * adaptiveSignal.positionSizeMultiplier;

    // Execute trades based on signal
    if (adaptiveSignal.action === 'buy' && usdcBalance > 0) {
      const maxPositionPct = adaptiveSignal.activeStrategy.maxPositionPct;
      const adjustedPositionPct = maxPositionPct * finalPositionMultiplier;
      const positionSize = usdcBalance * confidence * Math.min(adjustedPositionPct, 0.98); // Cap at 98%
  ...
  const run: StrategyRun = {
    id: uuidv4(),
    name: runName || 'Enhanced-Adaptive-Strategy',
    type: 'backtest',
    createdAt: Date.now(),
    startDate,
    endDate,
