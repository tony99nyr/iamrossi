...
import PriceChart from '../../eth-trading/components/PriceChart';
import HealthStatusPanel from '../../eth-trading/components/HealthStatusPanel';
import PortfolioPerformancePanel from '../../eth-trading/components/PortfolioPerformancePanel';
import RegimeDisplay from '../../eth-trading/components/RegimeDisplay';
import StrategySignalPanel from '../../eth-trading/components/StrategySignalPanel';
import TradeAnalyticsPanel from '../../eth-trading/components/TradeAnalyticsPanel';
import RiskManagementPanel from '../../eth-trading/components/RiskManagementPanel';

type TimeRange = 'all' | 'ytd' | '6m' | '3m' | '1m' | '14d' | '7d' | '48h';

interface TradingBotClientProps {
  asset: TradingAsset;
}

export default function TradingBotClient({ asset }: TradingBotClientProps) {
  ...
  const checkAuth = useCallback(async () => {
    try {
      const res = await fetch(`/api/trading/paper/status?asset=${asset}`, {
  ...
  const updateSession = async () => {
    ...
    try {
      setIsUpdating(true);
      setError(null);
      const res = await fetch(`/api/trading/paper/update?asset=${asset}`, {
        method: 'POST',
        headers: getAuthHeaders(),
        credentials: 'include',
      });
  ...
  const stopSession = async () => {
    try {
      setIsStopping(true);
      setError(null);
      const res = await fetch(`/api/trading/paper/stop?asset=${asset}`, {
        method: 'POST',
        headers: getAuthHeaders(),
        credentials: 'include',
      });
  ...
  useEffect(() => {
    ...
    const handleFocus = () => {
      // Refresh status when tab/window regains focus
      fetchStatus();
    };
  ...
  const formatAssetAmount = (amount: number) => {
    ...
    return `${amount.toFixed(decimals)} ${assetConfig.displayName}`;
  };
      ...
      <div className={stack({ gap: '16px' })}>
        <div className={flex({ justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' })}>
          <h1 className={css({ fontSize: '2xl', fontWeight: 'bold', color: '#e6edf3', display: 'flex', alignItems: 'center', gap: '8px' })}>
            <span>{asset === 'btc' ? '₿' : 'Ξ'}</span>
            <span>{assetConfig.displayName} Trading Bot - Paper Trading</span>
          </h1>
          <div className={flex({ gap: '8px', alignItems: 'center' })}>
            ...
            {session && (
              <Link
                href={`/tools/${asset}-trading/audit`}
                className={css({
                  padding: '8px 16px',
                  bg: '#21262d',
                  border: '1px solid #30363d',
                    ...
                    >
                      <div className={flex({ gap: '16px', alignItems: 'center' })}>
                        ...
                        <span className={css({ color: '#e6edf3' })}>
                          {formatAssetAmount(trade.ethAmount)} @ ${trade.ethPrice.toFixed(2)}
                        </span>
                      </div>
